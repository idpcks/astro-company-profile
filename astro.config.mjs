...
export default defineConfig({
  ...
  // Base path untuk deploy di subfolder ("https://site/rs/" -> base '/rs').
  // Kosong/'/' = deploy di root (domain/subdomain). Dipakai Astro untuk
  // nge-prefix SEMUA aset & route; link internal custom dilewati via
  // basePath()/stripBase() di src/i18n/index.ts supaya ikut base.
  base: ASTRO_BASE || '/',
  server: {
    host: true,
  },
  ...
  integrations: [
    ...
    sitemap({
      ...
      filter: (page) => !/(\/404|\/500|\/skeleton-demo)(\/)?$/.test(page),
    }),
  ],
  i18n: {
